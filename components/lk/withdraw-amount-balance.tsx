...
export const WithdrawAmountBalance: React.FC<Props> = ({ session, isSystem, value, className }) => {
    ...
    const handleWholeBalanceClick = () => {
        if (session?.user?.balance) {
            setCount(session.user.balance); // Устанавливаем баланс в состояние
        }
    };
    ...
    return (
        <div className="flex flex-col md:gap-5 gap-2 text-black bg-white border-[1px] border-[#f5f5f5] px-4 md:py-4 py-2 rounded-[10px]">
            ...
            <div className="flex flex-row items-center md:text-[18px] text-[13px] md:px-4 px-2 gap-3 border-[1px] border-[#b0b0b0] rounded-[5px]">
                <input
                    placeholder={`${t('typing')}`}
                    onChange={handleChange}
                    value={count} // Устанавливаем значение из состояния
                    className="w-[88%] bg-white border-transparent focus:outline-0"
                    type="number"
                />
                <h2
                    className="md:text-[15px] text-[12px] cursor-pointer text-blue-500"
                    onClick={handleWholeBalanceClick} // Добавляем обработчик клика
                >
                    {t('whole-balance')}
                </h2>
            </div>
