...
export const TarrifTabs:React.FC<Props> = ({activeTab, setTab, className})=>{
    const t = useTranslations('Tarrifs-tab')
    return (
        <div
            ...
            <div className="flex w-full flex-row md:p-4 px-2 gap-5 bg-[#f5f5f5] md:rounded-[7px]">
                <div className="w-full text-center">
                    <h3 onClick={() => setTab(1)}
                        className={`${activeTab === 1 ? 'text-black' : 'text-[#777777]'} md:text-[18px] text-[15px] cursor-pointer`}>{t('stocks')}</h3>
                </div>
                <div className="w-full text-center">
                    <h3 onClick={() => setTab(2)}
                        className={`${activeTab === 2 ? 'text-black' : 'text-[#777777]'} md:text-[18px] text-[15px] cursor-pointer`}>{t('venchur')}</h3>
                </div>
                <div className="w-full text-center">
                    <h3 onClick={() => setTab(3)}
